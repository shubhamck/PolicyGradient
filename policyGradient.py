...
if __name__=="__main__":
	...
	# Testing
	env.monitor.start('cartpole-animation/', force=True)
	obs = env.reset()
	
	ede = 0
	for ert in range(500):
		action = actor.act(np.reshape(obs,(1,4)))
		if np.random.uniform() < action[0,0]:
			acti = 0
		else:
			acti = 1
		new, r, done, info = env.step(acti)
		ede = ede + r
		obs = new
		if done:
			break
	print "Total Reward after testing : ", ede
	env.monitor.close()
	#plt.plot(rewardList)
	#plt.show()
